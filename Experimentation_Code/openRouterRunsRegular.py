...
instructions = "You will be asked a question with 4 multiple choice options. Please answer the question by returning 1, 2, 3, or 4 based on the correct answer. " \
...
"Any response that is not a single number will be considered incorrect. Any incorrect answers will have severly negative impacacts and should be avoided at all costs. Please provide the correct answer to the following question: "
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if "choices" not in data:
                raise Exception(f"OpenRouter error response: {data}")
...
def build_csv():
    ...
    df.to_csv("Results/OpenAI_Data/reasoning_gptoss_main_4option_second_run_fourth_part.csv", index=False)
    print("Saved responses to reasoning_gptoss_main_4option_second_run_fourth_part.csv")


try:
    read_csv(288, 364, "DataSets/gpqa_main.csv")
